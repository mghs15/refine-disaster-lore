...
Object.keys(GSI.MUNI_ARRAY).forEach( key => {

  const d = GSI.MUNI_ARRAY[key];
  const c = d.split(",");

  const pcode = c[0].padStart(2, "0")
  const pname = c[1];
  prefNames[pcode] = pname;
  
  if(c[3].match(/区$/) && c[0] != "13") return;
  
  const code = c[2].padStart(5, "0")
  const name = c[3];
  muniDataset[code] = {
    name: name,
    code: code,
...
//console.log(muniDataset);


module.exports = {
  GSI: GSI,
  prefNames: prefNames,
  muniDataset: muniDataset
};
